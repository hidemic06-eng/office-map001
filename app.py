...
if os.path.exists(FILENAME):
    ...
    # マップコンテナの背景に少し影を付けて浮き上がらせる
    map_html = f'''
    <div style="position: relative; width:100%; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 15px rgba(0,0,0,0.3);">
        <img src="data:image/png;base64,{b64_string}" style="width:100%; opacity:0.85; display: block;">
    '''
    
    for seat_id, pos in seat_coords.items():
        occ = df_now[df_now["座席番号"] == seat_id]
        
        # デザイン変数：在席か空席かで色と光り方を変える
        if not occ.empty:
            color = "#FF4B4B" # モダンな赤
        ...
        # Coolドット（ネオン風）
        map_html += f'''
            <div style="position: absolute; width:12px; height:12px; border-radius: 50%; 
                        top:{pos["top"]}%; left:{pos["left"]}%; 
                        background-color:{color}; border:2px solid white; 
                        box-shadow: 0 0 8px {glow}; z-index:5; transform: translate(-50%, -50%);">
            </div>'''
        ...
        if label:
            map_html += f'''
                <div style="position: absolute; top:{pos["top"]}%; left:{pos["left"]}%; 
                            font-size:10px; font-weight:bold; background:rgba(33, 33, 33, 0.9); 
                            color:white; padding:2px 8px; border-radius:4px; 
                            transform:translate(-50%, -150%); white-space:nowrap; z-index:10;
